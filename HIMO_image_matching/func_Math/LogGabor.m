...
logGabor = zeros(rows, cols, nscale);
for s = 1:nscale
    wavelength = minWaveLength*mult^(s-1);
    fo = 1.0/wavelength;
    logGabor(:,:,s) = exp((-(log(radius/fo)).^2) / (2 * log(sigmaf)^2));
    logGabor(:,:,s) = logGabor(:,:,s).*lp;
    logGabor(1,1,s) = 0;  % log friendly
end
clear radius lp

angle = (0:norient-1)*pi/norient;
cos_a = reshape(cos(angle), 1,1,[]);
sin_a = reshape(sin(angle), 1,1,[]);
ds = sintheta .* cos_a - costheta .* sin_a;
dc = costheta .* cos_a + sintheta .* sin_a;
dtheta = abs(atan2(ds,dc)); clear ds dc
dtheta = min(dtheta*norient/2,pi);
spread = (cos(dtheta)+1)/2; clear dtheta
